...
export class ReadNoteTool implements Tool {
  ...
  getDeclaration(): FunctionDeclaration {
    return {
      name: "read_note",
      description:
        "Fetches a note's structure, links, and backlinks. Content is also available upon request. Prioritize 'structure' to save tokens and quickly understand organization.",
      parameters: {
        type: Type.OBJECT,
        properties: {
          noteTitle: {
            type: Type.STRING,
            description:
              "The title or path of the note to fetch. Must be a note mentioned in the context (e.g., in links or backlinks of already read notes).",
          },
          detail: {
            type: Type.STRING,
            description:
              "The level of detail to fetch. 'structure' fetches headings and links. 'text' fetches everything.",
            enum: ["structure", "text"],
          },
        },
        required: ["noteTitle"],
